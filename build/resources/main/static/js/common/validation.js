 ...
 * valTitle = 한글명
 * valType = valid 타입 체크 input type 속성 보다 우선순위 없는 경우 type 속성 적용 * 입력 이벤트 조정
 * valPattern = form 어트리뷰트 체크 필요 속성 상수 VALIDPATTERN 패턴 참조 미등록시 valType 적용
 * notNull = Not Null 허용 가능
 * maxL = 최대 길이(radio, checkbox 대응 속성으로 input 사용 시 maxlength, minlength 속성 강제 적용) * valPattern 길이 우선
 * minL = 최소 길이(radio, checkbox 대응 속성으로 input 사용 시 maxlength, minlength 속성 강제 적용) * valPattern 길이 우선
 */
...
let defaultPattern = {
    id	: ["^[a-zA-Z0-9_-]{{?},{?}}$","영문+숫자+기호['-','_']형식의 {?}~{?}자리", [6, 30]],
    pwd : ["^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[*.!@$%^&(){}:<>,?/~_+-=|])[A-Za-z\\d*.!@$%^&(){}:<>,?/~_+-=|]{{?},{?}}$","최소 하나 이상의 영문(대소문자),숫자,기호 조합 형식의 {?}~{?}자리", [8, 30]],
...
class Validation {
    ...
    constructor(form, fields, option = {}) {
        ...
        this._submitAction = function() { this._form.submit(); };
    ...
    setInputValid(field) {
        ...
        let valPattern = field.getAttribute('valPattern') ? field.getAttribute('valPattern') : field.getAttribute('valType');
        ...
        if (this._valPattern[valPattern]) {
            ...
            if (field.getAttribute('minL')) field.setAttribute('minlength', field.getAttribute('minL'));
            if (field.getAttribute('maxL')) field.setAttribute('maxlength', field.getAttribute('maxL'))
        }
    }

    valTypeCheck(field) {
        return field.getAttribute('valType') ? field.getAttribute('valType') : field.getAttribute('type');
    }
    ...
    validationSubmit() {
        this._form.addEventListener('submit', (e) => {
            ...
            error = this._fields.some((field) => {
                ...
                let result = true;
                let valName = (selector.getAttribute('valTitle')) ? selector.getAttribute('valTitle') : selector.getAttribute('name');
                let valType = this.valTypeCheck(selector);
                let tagType = this.tagTypeCheck(selector);
                let minL = 0;
                let maxL = 0;
                if (selector.getAttribute('minL') || selector.getAttribute('minlength'))
                    minL = (selector.getAttribute('minL')) ? selector.getAttribute('minL') : selector.getAttribute('minlength');
                if (selector.getAttribute('maxL') || selector.getAttribute('maxlength'))
                    maxL = (selector.getAttribute('maxL')) ? selector.getAttribute('maxL') : selector.getAttribute('maxlength');
                ...
                if (selector.getAttribute('NNull') == 'true' && !result) {
