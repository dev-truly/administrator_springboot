...
function _TabMenuMainClass(CreateName ,url)
{
	...
	this.addCount = _TabAddCount;	// 탭영역 초과 계산 및 노출
	this.sel= _TabSel;			//목록
...
function _iframeSize(id) {
	...
	obj.style.height = String(ifHeight+70)+"px";
...
function _TabAdd(tabmenu)
{
	...
	else
	{
		...
		{
			alert("최대 텝 사용 " +this.m_max + "개를 모두 사용하고 있습니다.\n다른 텝을 닫고 사용하기시 바랍니다.");
			return;
		}
...
{
	var obj = this.m_tabList[menuidx];2015-06-10
	if(document.getElementById(this.m_tabID) != null)
	{
		//document.getElementById(this.m_tabID).insertAdjacentHTML("afterBegin" ,"<li id='menuli_"+obj.m_id+"' class='menu-tab1'><div class='m-tab-on' id='menu_tab_"+obj.m_id+"'><p class='m-tab-on-l' id='menu_tab_pl"+obj.m_id+"'>&nbsp;</p><p class='m-tab-on-c' id='menu_tab_pc"+obj.m_id+"'><a onclick='eval(\""+this.m_createID+"\").focus(\""+obj.m_id+"\");' style='cursor:hand;'>"+obj.m_name+"</a><a href='javascript:Tabmenu.del();' id='menu_tab_img"+obj.m_id+"'><img src='/_admin/_images/icon/tab_close.png' class='png24'></a></p></div></li>") ;
		insertHTML("<li id='menuli_"+obj.m_id+"' class='menu-tab1' style='cursor:pointer;'><div class='m-tab-on' id='menu_tab_"+obj.m_id+"'><p class='m-tab-on-l' id='menu_tab_pl"+obj.m_id+"'>&nbsp;</p><p class='m-tab-on-c' id='menu_tab_pc"+obj.m_id+"'><a onclick='eval(\""+this.m_createID+"\").focus(\""+obj.m_id+"\");' style='cursor:hand;'>"+obj.m_name+"</a><a href='javascript:Tabmenu.del();' id='menu_tab_img"+obj.m_id+"'><img src='/resources/images/admin/icon/tab_close.png' class='png24'></a></p></div></li>",document.getElementById(this.m_tabID));
		//객체 넣기
...
function _TabAddCount() {
	var sumTabWidth = 0;
	var maxWidthTabCount = 0;
	var tab_area_width = Math.ceil($('#tabMenuArea').width());
	$('#tabMenuArea li').each(function(){
		sumTabWidth += Math.ceil($(this).width());
		if (tab_area_width < sumTabWidth) {
			maxWidthTabCount++;
		}
	});
	...
	if (maxWidthTabCount) {
		$('#addTabCount').remove();
		$('#tabControll').before('<div id="addTabCount" style="float:left;padding-top:15px;font-weight:bold;font-size:14px;">+' + maxWidthTabCount + '</div>');
	}
	else {
		$('#addTabCount').remove();
	}
...
function _TabSelView(fthis)
{
	...
	for(var i = Len-1 ; i >= 0 ; i--)
	{
		...
		if(this.Ontab ==obj.m_id )
			...
			OnStr = "color:#3d3d3d;";
		addStr += "<li ><div style='width:100%;overflow:hidden;text-overflow:ellipsis;cursor:hand;' ><nobr><a onclick='eval(\""+this.m_createID+"\").focus(\""+obj.m_id+"\");' style='cursor:pointer;font-size:12px; font-family:Gulim; line-height:18px; text-decoration:none;"+OnStr+"  '>"+obj.m_name+"</a></nobr></div></li>";
	}
	...
	if(document.getElementById(this.m_tabListID) == null)
	{
		...
		insertHTML("<div style='position:relative;z-index:1000'><div id='" +this.m_tabListID + "' style='position:absolute;top:80px;left:87%;width:180px;min-height:100px;background-color:#ffffff;border:solid 2px #5357b2;overflow-x:hidden;padding: 5px;' ><ul  style='margin-left:5px;'>"+addStr+"</ul></div></div>",document.body);
	}
	else
	{
...
function _TabFocus(tabid)
{
	...
	if(document.getElementById("menuli_" +this.Ontab ) != null )
	{
		...
		document.getElementById("menu_tab_img" +this.Ontab ).innerHTML = "";
...
//			document.getElementById("ExtendTitle" + this.searchObject(this.Ontab).m_parentID).className = "off";
	...
	if(document.getElementById("menuli_" +tabid )  != null)
	{
		...
		document.getElementById("menu_tab_img" +this.Ontab ).innerHTML = "<img src='/resources/images/admin/icon/tab_close.png' class='png24'>";
		if(SelObj != null)
		{
			//텝위치조정
